...
import com.widedot.to8.gamebuilder.util.FastMath;
...
public class LAB
{
	...
	public static double ciede2000(LAB x, LAB y) {
		...
		double hp1 = FastMath.atan2_Op_2(b1, ap1); if (hp1 < 0) hp1 += 2*pi;
		double hp2 = FastMath.atan2_Op_2(b2, ap2); if (hp2 < 0) hp2 += 2*pi;
	...
	public static double[] aRGB2Lab(int[] image) {
		
		double[] imageLab = new double[image.length*3];
		double r, g, b, x, y, z;
		
		// D65 standard referent
		double X = 0.950470, Y = 1.0, Z = 1.088830;
		
		for(int i = 0; i < imageLab.length; i+=3){
			// if pixel is not transparent
			if ((((image[i] >> 24) & 0xFF) / 255.0) > 0) {
				// first, normalize RGB values
				r = ((image[i] >> 16) & 0xFF) / 255.0;
				g = ((image[i] >> 8)  & 0xFF) / 255.0;
				b = ((image[i] >> 0)  & 0xFF) / 255.0;
				...
				imageLab[i] = 116*y - 16;
				imageLab[i+1] = 500*(x-y);
				imageLab[i+2] = 200*(y-z);
			} else {
				imageLab[i] = 0;
				imageLab[i+1] = 0;
				imageLab[i+2] = 0;
