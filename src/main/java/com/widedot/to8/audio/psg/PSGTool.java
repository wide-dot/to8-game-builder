package com.widedot.to8.audio.psg;

import java.io.File;
import java.io.FileOutputStream;
import java.io.IOException;

public class PSGTool {
  public static final String TITLE = PSGTool.class.getSimpleName();
  ...
  private static void convertFile(File paramFile) {
    try {
      ...
      int[] arrayOfInt = PSGTools.getPSGdata(vGMInterpreter, framerate);
      if (arrayOfInt.length == 53248) {
        ...
        while (true) {
          ...
          exportSound(compress ? PSGTools.compressPSG(arrayOfInt) : arrayOfInt, new File(String.valueOf(outputFilename) + b++));
          arrayOfInt = PSGTools.getPSGdata(vGMInterpreter, framerate);
          if (arrayOfInt.length != 53248) {
      ...
      vGMInterpreter.close();
      arrayOfInt = maskData(arrayOfInt, mask);
      exportSound(compress ? PSGTools.compressPSG(arrayOfInt) : arrayOfInt, new File(outputFilename));
  ...
  public static void main(String[] paramArrayOfString) {
    ...
    if (inputFilename == null) {
      System.err.println("Usage: java -jar PSGTool.jar INPUTFILE [OUTPUTFILE] [2|3|23] [OPTIONS]");
    ...
    if (outputFilename == null)
      if (inputFilename.endsWith(".vgm")) {
        outputFilename = inputFilename.replace(".vgm", ".psg");
      } else {
        outputFilename = String.valueOf(inputFilename) + ".psg";
      }  
